"""E-015.3 controlled declarative theme scaffold generation tests."""

from __future__ import annotations

from pathlib import Path

import pytest
from typer.testing import CliRunner

from Engineering.cli.app import app
from Engineering.CodeGeneration import OverwritePolicy, ProjectGenerationInfo
from Engineering.core.exceptions import ThemeError
from Engineering.Templates import TemplateCategory, built_in_definition_repository
from Engineering.ThemeSystem import (
    THEME_MANIFEST_NAME,
    ThemeAppearance,
    ThemeManifestReader,
    ThemeScaffoldRequest,
    ThemeScaffoldService,
)


def _project() -> ProjectGenerationInfo:
    return ProjectGenerationInfo("Project", "P", "1.0.0", "Company", "MPL-2.0")


def _service(root: Path) -> ThemeScaffoldService:
    return ThemeScaffoldService.built_in(root, _project())


def _request(**changes: object) -> ThemeScaffoldRequest:
    values: dict[str, object] = {
        "theme_id": "example.slate",
        "name": "Slate Theme",
        "description": "A declarative slate theme.",
        "default_appearance": "light",
        "appearances": ("light", "dark"),
    }
    values.update(changes)
    return ThemeScaffoldRequest(**values)  # type: ignore[arg-type]


def test_builtin_theme_template_is_valid_and_bounded() -> None:
    definition = built_in_definition_repository().resolve("theme.declarative-basic")

    assert definition.metadata.category == TemplateCategory.THEME
    assert tuple(item.relative_path for item in definition.artifacts) == (
        "theme-manifest.yaml",
        "README.md",
    )


def test_generates_valid_scaffold_through_e009_and_e008(tmp_path: Path) -> None:
    result = _service(tmp_path).generate(_request())
    root = tmp_path / "Themes" / "example-slate"

    assert result.execution.report.success
    assert result.destination == "Themes/example-slate"
    assert result.execution.manifest is not None
    assert result.execution.manifest.verify(root).passed
    manifest = ThemeManifestReader().read(root / THEME_MANIFEST_NAME)
    assert manifest == result.theme_manifest
    assert manifest.default_appearance == ThemeAppearance.LIGHT
    assert tuple(item.appearance for item in manifest.palettes) == (
        ThemeAppearance.DARK,
        ThemeAppearance.LIGHT,
    )
    readme = (root / "README.md").read_text(encoding="utf-8")
    assert "does not" in readme
    assert "inject CSS" in readme


def test_dry_run_writes_nothing(tmp_path: Path) -> None:
    result = _service(tmp_path).generate(_request(dry_run=True))

    assert result.execution.report.success
    assert result.execution.report.dry_run
    assert result.execution.manifest is None
    assert not (tmp_path / "Themes").exists()


def test_conflicts_by_default_and_overwrites_only_when_explicit(tmp_path: Path) -> None:
    service = _service(tmp_path)
    first = service.generate(_request())
    assert first.execution.report.success
    readme = tmp_path / "Themes" / "example-slate" / "README.md"
    readme.write_text("user content\n", encoding="utf-8")

    conflict = service.generate(_request())
    assert not conflict.execution.report.success
    assert readme.read_text(encoding="utf-8") == "user content\n"

    replaced = service.generate(_request(overwrite=OverwritePolicy.ALLOWED))
    assert replaced.execution.report.success
    assert "# Slate Theme" in readme.read_text(encoding="utf-8")


@pytest.mark.parametrize(
    "destination",
    (
        "outside",
        "Themes/nested/theme",
        "../Themes/example",
        "C:/Themes/example",
    ),
)
def test_rejects_destinations_outside_direct_theme_root(
    tmp_path: Path, destination: str
) -> None:
    with pytest.raises(ThemeError, match="direct child of Themes"):
        _service(tmp_path).generate(_request(destination=destination))


@pytest.mark.parametrize(
    ("changes", "message"),
    (
        ({"appearances": ("dark", "dark")}, "duplicate appearance"),
        ({"appearances": ("sepia",)}, "appearance must be one of"),
        ({"default_appearance": "sepia"}, "default appearance must be one of"),
        (
            {"default_appearance": "dark", "appearances": ("light",)},
            "must have a matching palette",
        ),
    ),
)
def test_rejects_invalid_scaffold_inputs(
    tmp_path: Path, changes: dict[str, object], message: str
) -> None:
    with pytest.raises(ThemeError, match=message):
        _service(tmp_path).generate(_request(**changes))


def test_cli_exposes_controlled_generation_and_dry_run() -> None:
    runner = CliRunner()
    help_result = runner.invoke(app, ["generate", "theme", "--help"])
    dry_run = runner.invoke(
        app,
        [
            "generate",
            "theme",
            "example.cli-theme",
            "--appearance",
            "light",
            "--appearance",
            "dark",
            "--dry-run",
        ],
    )

    assert help_result.exit_code == 0
    assert "--default-appearance" in help_result.output
    assert "--overwrite" in help_result.output
    assert dry_run.exit_code == 0
    assert "Dry-run Generation completed: 2 created" in dry_run.output
    assert "Destination: Themes/example-cli-theme" in dry_run.output


def test_cli_rejects_invalid_appearance() -> None:
    result = CliRunner().invoke(
        app,
        [
            "generate",
            "theme",
            "example.invalid-theme",
            "--appearance",
            "sepia",
            "--dry-run",
        ],
    )

    assert result.exit_code == 1
    assert "appearance must be one of" in result.output
