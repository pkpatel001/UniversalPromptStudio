...
| Desktop product | Tauri/Vite management library, theme selection, bundled Windows sidecar, verified installed IPC, versioned app-data SQLite, durable prompt management, local search, deterministic saved-block preview, confirmed offline echo execution | External provider settings/invocation, credentials/endpoints, execution history, workflow UI, non-Windows bundles, fuzzy/cross-project search, deletion recovery, distribution polish |
