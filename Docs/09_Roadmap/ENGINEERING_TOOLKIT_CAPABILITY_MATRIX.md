...
| Desktop product | Tauri/Vite management library, theme selection, bundled Windows sidecar, verified installed IPC, versioned app-data SQLite, durable project/prompt create/list/edit/delete, categories, tags, ordered blocks, project-scoped local search | Composer/execution UI, workflow UI, provider settings, non-Windows bundles, fuzzy/cross-project search, deletion recovery, distribution polish |
