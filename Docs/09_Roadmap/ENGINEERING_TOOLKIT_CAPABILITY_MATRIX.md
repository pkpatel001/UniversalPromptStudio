...
| Release | Local sdist, wheel, frontend ZIP, target-triple Python sidecar, unsigned Windows NSIS packaging; checksums and independent verification | Publishing, signing, Git tags/releases, MSI, updater metadata, non-Windows sidecars |
...
| Desktop product | Tauri/Vite management library, theme selection, bundled Windows sidecar, verified installed IPC, versioned app-data SQLite, durable prompt management, local search, deterministic saved-block preview, offline echo, controlled OpenAI Responses settings/invocation, Windows DPAPI credentials | Additional providers/endpoints/auth schemes, model discovery, streaming/retries, execution history, workflow UI, non-Windows bundles, fuzzy/cross-project search, deletion recovery, distribution polish |
